...
def net_transform(aug=None):
    ...
    def transform(data):
        ...
        img = img / 255 - 0.5
        return img.permute(2, 0, 1), clazz
...
class HistClassConfig(Config):
    ...
    def __init__(self, data_glob: str) -> None:
        def __transform(data):
            return data[0], data[1]['class']

        filename_transforms = {
            'wsi': lambda filename: filename.split('_')[0],
            'class': lambda filename: int(filename.split('_')[4][0])
        }
        self.__data = NumpyImageDataset(data_glob, add_info=True,
                                        filename_transforms=filename_transforms,
                                        transform=__transform)

        self.__keys = self.__data.filenames
        self.__groups = self.__data.info['wsi']
        self.__stamp = datetime.now()
        self.__criterion = nn.CrossEntropyLoss()
        super().__init__(Path(
            f'Jobs/{self.__stamp:%Y-%m-%d-%H-%M-%S}_PAN0_Class'))
    ...
    def model_config(self) -> ModelConfig:
        ...
        model = geffnet.efficientnet_b7(num_classes=5)
        return ModelConfig(
            model=model,
