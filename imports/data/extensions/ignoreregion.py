...
class IgnoreRegionExtension(AbstractExtension):
    ...
    **IMPORTANT** If chosen loss function penalize for high rate of false
    ...
    Apply to image masks. Only 2 channel data can be processed now
    ...
    def __call__(self, data: np.ndarray) -> np.ndarray:
        ...
        if data.shape[2] != 2:
            raise IndexError('This extension work on only two dimensional data')
        ...
        return data * (1 - ignore[:, :, np.newaxis])
